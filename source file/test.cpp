...
char* BaseConverter(float number, int base)
{
    ...
    if (Negative)
    {
        resultPtr[resultIndex++] = '-';
    }
    for (int i = index - 1; i >= 0; --i)
    {
        resultPtr[resultIndex++] = result[i];
    }
    if (index > 0)
    {
        resultPtr[resultIndex++] = '.';
        for (int i = 0; i < i; ++i)
        {
            resultPtr[resultIndex++] = frac[i];
        }
    }
    resultPtr[resultIndex] = '\0';
